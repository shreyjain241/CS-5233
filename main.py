		...
		for lambda2 in range (5,15):
			for num_terminals in range(1,6):
				...
				csv_writer.writerow([lambda1,lambda2,num_terminals, 
					mean(avg_waiting_time),stddev(avg_waiting_time),
					mean(customers_delayed),stddev(customers_delayed),
					mean(perc_5mins), stddev(perc_5mins),
					mean(perc_4mins),stddev(perc_4mins)])
